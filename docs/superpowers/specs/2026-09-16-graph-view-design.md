...
| 갱신      | **탭을 열 때 빌드 + 새로고침 버튼**              | 볼트 열 때 선빌드 · 파일 감시      | 볼트 여는 경로를 느리게 하지 않는다. `watch.ts` 는 `OWNER: A` 라 안 건드린다. 탭을 벗어났다 돌아오는 것은 "열 때"가 아니다 — 컴포넌트가 탭이 있는 동안 계속 떠 있어 재빌드가 안 걸린다(§8) |
...
src/renderer/features/graph/
  ...
  draw.ts          ctx 에 노드·엣지·라벨을 그린다   (순수 — 테스트 가능)
...
**탭을 바꿔도 컴포넌트는 언마운트되지 않는다 — 숨길 뿐이다.** 그래프 탭이 있는 한
`GraphView` 는 계속 떠 있고, 활성 탭이 아닐 때는 `hidden` prop 만 켜진다. 언마운트했다가
다시 마운트하면 볼트를 다시 훑고 노드를 다시 무작위 배치해야 하는데, 그러면 탭을 오갈
때마다 pan·zoom·노드 위치를 잃는다 — "그래프 보다가 노트 열고 돌아오기"가 이 기능의
핵심 동선이라 그 비용을 감당할 수 없다.

`requestAnimationFrame` 루프 자체는 멈추지 않는다 — 숨겨진 동안에도 계속 돌되, 매 프레임
`hidden` 을 제일 먼저 확인해 그 즉시 반환한다. **캔버스 크기(`host.clientWidth`)를 읽기
전에 반환해야 한다.** `display: none` 인 요소는 `clientWidth`·`clientHeight` 가 0 이라,
그 값으로 백버퍼를 키우면 캔버스가 0×0 이 되어 그림이 사라진다. 시뮬레이션(`sim`)은 이
가드와 무관하게 계속 돈다 — alpha 로 스스로 식으므로 숨겨진 동안 CPU 를 계속 먹지 않는다.
...
- **`draw`** — `ctx` 를 인자로 받으므로 기록용 스텁으로 jsdom 없이 찍는다(`draw.test.ts`, 5개)
  - **`translate` 를 `scale` 보다 먼저 건다** — `hit.toWorld` 가 이 순서의 역을 가정하므로,
    순서가 뒤집히면 기존 테스트는 전부 그대로 통과한 채 클릭만 엉뚱한 노드에 떨어진다
  - 줌이 라벨 문턱 아래고 강조가 없으면 라벨을 안 그린다
  - 줌이 문턱 이상이면 노드 제목을 그린다
  - 강조 밖의 노드는 옅게, 강조 안의 노드는 온전한 알파로 그린다
  - 좌표가 없는 노드와 그 끝점을 가진 엣지는 던지지 않고 건너뛴다
