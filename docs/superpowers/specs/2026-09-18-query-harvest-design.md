...
**`y/N` 프롬프트 (사용자 터미널).** 대화 1턴 → 빈 줄 → `근거 없는 문단 0개` · `위키에 반영? [y/N] y` → `3건 반영됨`.
AI 가 위키를 되풀이만 한 세션이라 모델이 **페이지 0장**을 냈고, 출처 페이지 · 로그 · `sync_state` 만 커밋됐다 — "처리됨" 이 남는 것이라 맞다.
같이 있던 미반영 로그가 `사용자 편집 봉인` 커밋에 실렸다 (§6 에 적은 귀결 그대로).
