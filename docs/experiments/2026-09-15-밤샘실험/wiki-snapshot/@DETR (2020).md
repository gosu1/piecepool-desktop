...
prediction with index σ(i) we define probability of class ci as ˆpσ(i)(ci) and
the predicted box as ˆbσ(i). With these notations we define Lmatch(yi, ˆyσ(i)) as
−1{ci6 =∅} ˆpσ(i)(ci) + 1{ci6 =∅}Lbox(bi, ˆbσ(i)).
...
i=1
[
− log ˆpˆσ(i)(ci) + 1{ci6 =∅}Lbox(bi, ˆbˆσ (i))
]
, (2)
where ˆσ is the optimal assignment computed in the first step (1). In practice, we
...
End-to-End Object Detection with Transformers 7CNN
set of image features
transformer
encoder
…
...
End-to-End Object Detection with Transformers 11self-attention(430, 600)
...
End-to-End Object Detection with Transformers 15Multi head attentionInput image
(3 x H x W)
Box embeddings
(d x N)
Encoded image
...
39. Rezatofighi, S.H., Kaskman, R., Motlagh, F.T., Shi, Q., Cremers, D., Leal-Taix´e,
...
sion d is a function with the following signature (using d′ = d
M , and giving
...
˜Xq = layernorm(Xq + dropout(LX′
q)) , (6)
where [;] denotes concatenation on the channel axis.
Single head An attention head with weight tensor T ′ ∈ R3×d′×d, denoted by
attn(Xq, Xkv, T ′), depends on additional positional encoding Pq ∈ Rd×Nq and
...
2(Xkv + Pkv); T ′
3Xkv] (7)
where T ′ is the concatenation of T ′
1, T ′
2, T ′
...
in our loss, together with a `1 loss on ˆb:
Lbox(bσ(i), ˆbi) = λiouLiou(bσ(i), ˆbi) + λL1||bσ(i) − ˆbi||1 , (9)
where λiou, λL1 ∈ R are hyperparameters and Liou(·) is the generalized IoU [38]:
Liou(bσ(i), ˆbi) = 1 −
( |bσ(i) ∩ ˆbi|
|bσ(i) ∪ ˆbi| − |B(bσ(i), ˆbi) \ bσ(i) ∪ ˆbi|
|B(bσ(i), ˆbi)|
)
. (10)
...
the loss sufficiently well-behaved for stochastic gradients. B(bσ(i), ˆbi) means the
...
in the first decoder layer can be skipped.Add & Norm
FFN
Add & Norm
...
ity O(d2HW +d(HW )2): O(d′d) is the cost of computing a single query/key/value
embeddings (and M d′ = d), while O(d′(HW )2) is the cost of computing the at-
...
End-to-End Object Detection with Transformers 2520 40 60 80 100
0
10
20
30
