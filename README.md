...
> **지금 상태: 7·8단계 진행 중.** 엔진(`src/core/`)은 아직 스텁이지만,
> 앱은 실행되고 사이드바 등 왼쪽 크롬은 실제 볼트로 동작한다.
...
| 이렇게 쓰면                      | 결과                                      |
| -------------------------------- | ----------------------------------------- |
| `import { x } from "./notes.ts"` | OK — `.ts` 확장자를 반드시 붙인다         |
| `import { x } from "./notes"`    | 죽는다 — Node 는 확장자를 추론하지 않는다 |
| `enum Kind { A, B }`             | 죽는다 — 유니온이나 `as const` 를 쓴다    |
| `constructor(readonly x: T) {}`  | 죽는다 — 필드를 따로 선언한다             |

`enum` 과 파라미터 프로퍼티는 lint 가, 빠진 확장자는 `tsc` 가 잡는다. 외우지 않아도 된다.
...
| LLM 쓰기    | JSON 제안 → 검증 → 앱이 반영      | **AI 는 JSON 을 내고, 코드가 검문하고 파일을 쓴다** | ADR-0002. "직접 편집" 은 실험에서 본문을 일기로 만들어 폐기 |
...
앱은 **한 명령으로 뜬다.** `scripts/dev.mjs` 가 preload 를 빌드하고 vite 를 띄운 뒤 electron 을 붙인다 —
창을 닫으면 dev 서버도 같이 내려간다.

```bash
npm start       # preload 빌드 → vite → electron
```

renderer 만 브라우저로 보고 싶으면 `npm run dev` 를 따로 쓴다.
...
제품 경로는 `npm run ingest -- <볼트> [파일]` 이다 (파일이 없으면 볼트 전체, 항목마다 커밋 하나). `scripts/demo/` 는 같은 엔진(`src/core/ingest/`)을 쓰는 **실험용 실행기**다 — 응답 캐시·예산 상한·정답 세트 비교(`analyze.ts` · `compare.ts`)가 붙어 있고 git 은 안 건드린다. 모델이나 프롬프트를 바꾸면 이걸로 지표를 다시 받는다.
...
```bash
cp .env.example .env                                              # PIECEPOOL_LLM_API_KEY (Kimi)
node --env-file=.env scripts/demo/index.ts --vault <내 노트 폴더> --bm25 --top 8  # wiki/ 가 그 폴더 안에 생긴다
```
...
끝나면 `fixtures/vault-life/` 를 **옵시디언으로 열어** `wiki/` 를 본다. 키를 안 넣고 결과만 보려면
[`docs/experiments/2026-09-15-밤샘실험/wiki-snapshot/`](docs/experiments/2026-09-15-밤샘실험/wiki-snapshot/) 에 5회차 결과 59장이 있다.
...
| ---- | --------------------------- | -------------------------------------------------------------------------------------- |
| 0    | 레포 골격                   | **완료**                                                                               |
| 1    | `vault/` — 볼트 읽기·쓰기   | 열기·트리·원문 읽기·경로 방어. 쓰기는 아직 (데모에 논리가 있다)                        |
| 2    | `index/` — 링크 파싱·백링크 | `[[링크]]` 파싱·해석·볼트 스캔. 백링크·캐시·감시는 아직                                |
| 3    | `git/` — 커밋과 되돌리기    | **완료.** init·봉인·경로 커밋·되돌리기(계획 → 고름 → 적용). ingest 에 꽂는 건 4단계    |
| 4    | `ingest` + `agent`          | **완료.** 데모 엔진을 `src/core/ingest/` 로 옮겨 git 안전망에 꽂았다. `npm run ingest` |
| 5    | `lint` · `processInbox`     |                                                                                        |
| 6    | 쿼리 세션 + 수확            |                                                                                        |
| 7    | Electron 셸                 | 창·메뉴·preload·IPC. API 키 저장은 아직                                                |
| 8    | 편집 UI · 그래프 뷰         | 왼쪽 크롬 · 탭 · 위키 읽기 · 탭 분할 · 그래프 뷰. 에디터는 아직                        |
...
| `renderer/`·`preload/` → `core/`·`main/` import 금지                       | renderer 는 preload 가 열어준 것만 쓴다 |
| `enum`·`namespace`·파라미터 프로퍼티 금지 (lint) · `.ts` 확장자 필수 (tsc) | `node` 가 `.ts` 를 직접 실행하므로      |
